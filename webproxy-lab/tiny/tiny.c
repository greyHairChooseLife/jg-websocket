...
void doit(int fd)
{
  ...
  if (stat(filename, &sbuf)< 0) {
    clienterror(fd, filename, "404", "NOT FOUND", "Tiny couldn't find this file >3<");
    return;
  }
  
  if (is_static) {
    if (!(S_ISREG(sbuf.st_mode)) || !(S_IRUSR & sbuf.st_mode)) {
      clienterror(fd, filename, "403", "Forbidden", "Tiny can't fucking read this file, sit down");
      return;
    }
...
void read_requesthdrs(rio_t *rp) 
/* HTTP 프로토콜 규칙을 지키기 위해 헤더 끝의 빈 줄까지 읽어야 요청이 정상적으로 처리가 된다. */
